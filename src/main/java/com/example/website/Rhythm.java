...
//        for (String s : wordTable.values().stream().filter(Objects::nonNull).collect(toSet())) {
        for (String s : Dictionary.getValuesSet()) {
...
//                ok=ok.replaceFirst("[^ёуеыаоэяию]","@");
    ...
    public static Map<String, Set<String>> get(int key, String chr) {
        Map<String, Set<String>> rhythm = new TreeMap<>(Comparator.comparing(Utils::reverseOk));
